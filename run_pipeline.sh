...
if [[ "$CLEANUP_CHOICE" == "j" || "$CLEANUP_CHOICE" == "J" ]]; then
    echo -e "${YELLOW}Lösche alte Konfigurationen...${NC}"; rm -f src/utbot2/strategy/configs/config_*.json; rm -f artifacts/results/optimization_results.json; echo -e "${GREEN}✔ Aufräumen abgeschlossen.${NC}"
else
