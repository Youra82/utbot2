...
def run_optimization(start_date, end_date, timeframes_str, symbols_list, risk_percent=None, initial_capital=1000, top_n=10):
    ...
    for symbol_arg in symbols_to_optimize:
        ...
        # (Benchmark-Teil bleibt unverändert)
        try:
        ...
        for i, row in top_results.reset_index(drop=True).iterrows():
            ...
            # --- HIER IST DIE ANPASSUNG AN DER TABELLE ---
            ...
            if isinstance(trade_history, list) and trade_history:
                print("    ------------------------------------------------------------------------------------------")
                print("    | Zeitpunkt           | Typ   | Gewinn (USDT) | Grund         | Hebel | Neuer Kontostand     |")
                print("    ------------------------------------------------------------------------------------------")
                ...
                for trade in display_trades:
                    ...
                    leverage_str = f"{trade.get('leverage_used', 0.0):.1f}x".rjust(5)
                    balance_str = f"{trade['account_balance']:.2f} USDT"
                    print(f"    | {trade['exit_time']} | {side:<5} | {pnl_usdt_str} | {exit_reason_str} | {leverage_str} | {balance_str:>20} |")
    ...
    if len(all_symbols_results_list) > 0:
        ...
        if final_sorted is not None:
            ...
            for i, row in final_top_10.reset_index(drop=True).iterrows():
                platz = i + 1
                # (Die Ausgabe der finalen Gesamtwertung bleibt unverändert)
        else:
