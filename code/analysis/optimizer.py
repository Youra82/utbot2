...
# (Die Hilfsfunktionen bleiben gleich)
def run_single_optimization_pass(param_combinations, base_params, initial_capital, data_cache, start_date, end_date):
    ...
    for timeframe, data in data_cache.items():
        ...
        for params_to_test in param_combinations:
            ...
            data_with_signals = calculate_signals(data.copy(), current_params, start_date, end_date) # Übergibt start/end
            result = run_backtest(data_with_signals, current_params, initial_capital=initial_capital, start_date=start_date, end_date=end_date)
...
def run_optimization(start_date, end_date, timeframes_str, symbols_list, risk_percent=None, initial_capital=1000, top_n=10):
    ...
    for symbol_arg in symbols_to_optimize:
        ...
        param_grid = {
            ...
            'ltf_vol_sensitivity': [1.0, 1.5] # NEUER PARAMETER
        }
        ...
        for i, row in top_results.reset_index(drop=True).iterrows():
            ...
            print(f"    Vola-Sensitivität:  {row['ltf_vol_sensitivity']:.1f}") # NEUE ANZEIGE
    ...
    # (Finale Gesamtauswertung bleibt gleich)
