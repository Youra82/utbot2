...
def run_backtest(data, params, initial_capital=1000, verbose=True):
    ...
    position_side = None
    entry_price = 0.0
    trailing_stop_price = 0.0 # Kombiniert SL und TP
    peak_price = 0.0
    ...
    for i in range(1, len(data)):
        ...
        if in_position:
            ...
            # --- NEUE VEREINHEITLICHTE TRAILING STOP LOGIK ---
            if position_side == 'long':
                peak_price = max(peak_price, current_candle['high'])
                potential_new_stop = peak_price * (1 - trailing_stop_percent / 100)
                trailing_stop_price = max(trailing_stop_price, potential_new_stop) # Stop kann nur steigen
                ...
                potential_new_stop = peak_price * (1 + trailing_stop_percent / 100)
                trailing_stop_price = min(trailing_stop_price, potential_new_stop) # Stop kann nur fallen
                if current_candle['high'] >= trailing_stop_price:
                    exit_price, exit_reason = trailing_stop_price, "TRAIL-STOP"
            
            # Ausstieg durch Gegensignal (falls TSL nicht ausgelöst wurde)
            if exit_price == 0:
                if (position_side == 'long' and prev_candle['sell_signal_ut']) or \
                   (position_side == 'short' and prev_candle['buy_signal_ut']):
        ...
        if not in_position and account_balance > 0:
            ...
            if side_candidate:
                ...
                if side_candidate == 'long':
                    initial_sl_price = entry_price - (atr_for_sl * sl_multiplier)
                else:
                    initial_sl_price = entry_price + (atr_for_sl * sl_multiplier)
                ...
                in_position = True
                position_side = side_candidate
                peak_price = entry_price
                trailing_stop_price = initial_sl_price # Startpunkt des Trailing Stops
...
# load_data_for_backtest bleibt unverändert
