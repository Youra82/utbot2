    ...
    for i in range(1, len(data)):
        ...
        if in_position:
            ...
            if position_side == 'long':
                ...
                tp_trigger_price = peak_price * (1 - trailing_tp_percent / 100)
                if current_candle['low'] <= tp_trigger_price:
                    exit_price, exit_reason = tp_trigger_price, "TRAIL-TP"
            elif position_side == 'short':
                peak_price = min(peak_price, current_candle['low'])
                tp_trigger_price = peak_price * (1 + trailing_tp_percent / 100)
                if current_candle['high'] >= tp_trigger_price:
                    exit_price, exit_reason = tp_trigger_price, "TRAIL-TP"
            ...
            if exit_price > 0:
                ...
                trade_history.append({
                    ...
                    'exit_reason': exit_reason # HIER WIRD DER GRUND GESPEICHERT
                })
                in_position = False
                position_size_coins = 0.0
...
def load_data_for_backtest(symbol, timeframe, start_date_str, end_date_str, hide_messages=False):
    # ... (Code von letzter Version bleibt identisch)
