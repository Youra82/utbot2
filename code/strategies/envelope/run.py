...
# Alle bestehenden Orders vorab stornieren (optional, aber oft sicherer)
# cancel_all_orders() # Aktivieren, wenn gewünscht
...
def calculate_ut_signals(data, params):
    ...
    for i in range(1, len(data)):
        # TradingView: crossover(ema, stop) -> (ema > stop) and (ema[1] <= stop[1])
        buy_condition = (
            data['ema1'].iloc[i] > data['x_atr_trailing_stop'].iloc[i] and
            data['ema1'].iloc[i-1] <= data['x_atr_trailing_stop'].iloc[i-1]
        )
        
        # TradingView: crossunder(ema, stop) -> (ema < stop) and (ema[1] >= stop[1])
        sell_condition = (
            data['ema1'].iloc[i] < data['x_atr_trailing_stop'].iloc[i] and
            data['ema1'].iloc[i-1] >= data['x_atr_trailing_stop'].iloc[i-1]
        )
...
# 1. Prüfung: Ist der Tracker-Status in Ordnung?
...
# 2. Prüfung: Haben wir überhaupt ein gültiges Signal?
...
# 3. Prüfung: Gibt es eine offene Position?
if open_position:
    ...
    # Fall A: Gegenläufiges Signal -> Position schließen
    if (position_side == 'long' and sell_signal) or (position_side == 'short' and buy_signal):
        ...
        try:
            ...
            sys.exit() # Beenden, da der Zustand unklar ist
    # Fall B: Signal in die gleiche Richtung -> Nichts tun
    else:
...
# Wenn wir hier ankommen, haben wir KEINE offene Position und ein gültiges Signal.
# Nun folgen die finalen Prüfungen vor der Order-Platzierung.

# 4. Prüfung: Sind die entsprechenden Trades (Long/Short) überhaupt aktiviert?
...
# FINALE AKTION: Neue Position eröffnen
action_taken = "Keine Aktion durchgeführt"
if buy_signal and params['use_longs']:
    try:
        current_price = data.iloc[-1]['close']
        # Umrechnung der USDT-Größe in die Basiswährung (z.B. BTC)
