# tests/test_workflow.py (FINAL - BEREINIGT & OHNE MOCKING FÜR CCXT)
import pytest 
import os
import sys
import json
...
class MockGeminiModel:
    ...
    def generate_content(self, prompt, generation_config=None, safety_settings=None):
        ...
        if "aktueller_preis" in prompt:
             try:
                 current_price = float(prompt.split("aktueller_preis='")[1].split("'")[0])
             except:
                 pass
...
def test_setup(mock_fetch_ohlcv): # Das Argument MUSS im Code bleiben, da der Patch es injiziert, aber Pytest muss es ignorieren
